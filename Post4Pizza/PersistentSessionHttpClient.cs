...
namespace Post4Pizza
{
    public class PersistentSessionHttpClient
    {
        ...
        public string GetString(HttpRequestMessage requestMessage)
        {
            var result = httpClient.SendAsync(requestMessage).Result.Content.ReadAsStringAsync().Result;
            return result;
        }

        public string GetString(HttpRequestMessage requestMessage, params KeyValuePair<string, string>[] urlParameters)
        {
            ...
            return GetString(requestMessage);
        }
